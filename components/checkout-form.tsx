...
export default function CheckoutForm({ plan }: CheckoutFormProps) {
  ...
  const { trackPurchase, trackEvent, trackAddToCart, trackInitiateCheckout } =
    useFacebookPixel();
  ...
  const trackIREConversionCrossBrowser = async (
    conversionData: any
  ): Promise<boolean> => {
          ...
          ire(
            ...
            },
            {
              ...
              callback: function (response: any) {
                console.log(
                  `✅ IRE conversion successful (attempt ${attempt})`,
                  response
                );
                resolve(true);
              },
      ...
      if (createAccount) {
        ...
        if (createAccount) {
          try {
            ...
            console.log(
              "✅ Arrivia member created successfully:",
              arriviaResult
            );
          } catch (arriviaError) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.paymentIntent?.status === "succeeded") {
        ...
        setPaymentError(
          "Payment completed with unexpected status. Please contact support."
        );
        setIsProcessingPayment(false);
      }
    } catch (error) {
  ...
  const handleCreateAccountChange = (checked: boolean) => {
    ...
    trackEvent(checked ? "Lead" : "ViewContent", {
      ...
      content_name: checked
        ? "Create Account Selected"
        : "Create Account Deselected",
      content_ids: [plan.id],
    });
  };
        ...
        <div className="lg:col-span-2">
          <Card className="sticky top-24 bg-[#F7FAFF]  border border-[#E9F0FF]">
            ...
            <CardContent className="space-y-6">
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">
                    ${plan.discountedPrice} <del>${plan.regularPrice}</del>
                  </span>
                </div>
                {discount > 0 && (
              ...
              <div className="text-xs italic">
                <em>
                  Memberships automatically renew annually, every 365 days. To
                  cancel before your next billing cycle, please email support@approvedexperiences.com
                </em>
              </div>
                ...
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <span
                      className={`${
                        promoCode
                          ? "-top-3 left-2 scale-[0.9] dark:bg-black bg-white px-[4px]"
                          : "left-5 top-3"
                      } absolute dark:peer-focus:bg-darkBgColor peer-focus:-top-3 peer-focus:bg-white dark:text-[#757575] peer-focus:left-2 peer-focus:scale-[0.9] peer-focus:text-[#3B9DF8] text-[#757575] peer-focus:px-1 transition-all duration-300`}
                    >
                      Promo Code
                    </span>
                  </div>
                  <Button
                    ...
                    disabled={
                      !promoCode || promoCode.trim() === "" || promoApplied
                    }
